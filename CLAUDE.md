...
| **`rules-registry-effect`** | The GLOBAL registry: aggregates all 98 rules (96 per-file SYN/TYP/CFG + 2 GRAPH) into the two registries the engine consumes (`ruleRegistry` + `graphRuleRegistry`). Hand-assembled aggregator (replaces the legacy codegen `rule-registry.generated.ts`). | `registry.ts`, `index.ts` |
...
**Catalog: 98 rules across 14 categories** (the authoritative aggregation is `rules-registry`'s `registry.ts`; each rule has a colocated `src/test/*.test.ts`). Tier breakdown:
- **SYN (74):** type-safety 6, type-assertions 12, generics 4, async 4, exhaustiveness 3, error-handling 6, naming-idioms 16, security 6, module-boundaries 3, declaration-api 4, type-performance 3, functional-patterns 7. AST-only, always run. (The 4 CFG strictness rules are counted under CFG below.)
...
- **Catalog expansion** — all four tiers (SYN/TYP/CFG/GRAPH) are proven and all 14 categories populated (98 rules); author the rest of the spec taxonomy against the existing seams.
