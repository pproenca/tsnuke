...
These tests **define "done"** for the Effect-TS rewrite of `tsnuke`'s versioned
...
slice (`@tsnuke/score-effect`) for the monorepo MIN score (RULE-003) and band
label (RULE-002). The legacy module is the oracle
(`legacy/tsnuke/packages/core/src/build-report.ts`, read-only).
...
Vitest must transpile the `.ts`-entry `@tsnuke/score-effect` dependency at test
time; `vitest.config.ts` sets `test.server.deps.inline: ["@tsnuke/score-effect"]`
