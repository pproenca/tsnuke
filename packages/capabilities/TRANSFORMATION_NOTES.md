...
Strangler-fig slice produced by `/code-modernization:modernize-transform ts-doctor capabilities effect`.
Source (READ-ONLY): `legacy/ts-doctor/packages/ts-doctor-rules/src/capabilities.ts`
(72 lines; + the `RuleMeta`/`Severity`/`Capability` contract from
`packages/ts-doctor-rules/src/types.ts`). Target: `modernized/capabilities/effect/`.
...
- **Only the activation-relevant `RuleMeta` subset was vendored.** The predicate
  ...
  on. They belong to the full `@ts-doctor/rules` contract (Follow-up #1).
...
1. **De-vendor `RuleMeta` — DONE.** Now imports the canonical
   ...
   `@ts-doctor/contracts-effect` (the consolidation slice that owns the FULL contract).
   ...
   consumer (22/22). Dep `@ts-doctor/contracts-effect": file:../../contracts/effect`
