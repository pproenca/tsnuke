 ...
 *   - an uncaught error → exit 1 with `tsnuke: <message>` on stderr (terse, like
 ...
 * `tsnuke: <message>` + exit 1. `@effect/cli` `ValidationError`s (bad flags, RULE-028
...
const program = run(process.argv).pipe(
  Effect.catchAllCause((cause) =>
    Effect.sync(() => {
      ...
      // cause here only produced a redundant `tsnuke: Error: {…JSON…}` line — so for a
      ...
      process.stderr.write(`tsnuke: ${message}\n`);
      process.exitCode = 1;
    }),
  ),
);
