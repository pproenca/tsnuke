/**
 * Full end-to-end `tsnuke <dir>` runs through the `@effect/cli` command tree against a
...
beforeEach(() => {
  dir = mkdtempSync(join(tmpdir(), "tsnuke-e2e-"));
...
const runCli = async (argv: readonly string[]): Promise<string> => {
  ...
  await Effect.runPromise(
    run(["node", "tsnuke", ...argv]).pipe(Effect.provide(env.layer)),
  );
  return env.output.join("");
};

describe("end-to-end: tsnuke <dir>", () => {
  ...
  it("install subcommand dispatch → writes stub files, exit 0", async () => {
    ...
    expect(readFileSync(join(dir, ".agent/skills/tsnuke/SKILL.md"), "utf8")).toContain(
      "name: tsnuke",
    );
  ...
  it("--help (auto-help, a new @effect/cli capability) succeeds, doesn't crash", async () => {
    ...
    const exit = await Effect.runPromiseExit(
      run(["node", "tsnuke", "--help"]).pipe(Effect.provide(env.layer)),
    );
  ...
  it("an unknown flag is rejected by the POSIX parser", async () => {
    const env = e2eLayer();
    const exit = await Effect.runPromiseExit(
      run(["node", "tsnuke", "--definitely-not-a-flag"]).pipe(Effect.provide(env.layer)),
    );
...
describe("regression: boolean tri-state defaults (real argv parse)", () => {
  ...
  const writeSuppressedAnyProject = (): void => {
    ...
    writeFileSync(
      join(dir, "src", "bad.ts"),
      "// tsnuke-disable-next-line no-explicit-any\n" +
        "export function f(x: any): number {\n  return Number(x);\n}\n",
    );
  };
  ...
  it("--deep + --no-deep is rejected by the parser (mutual exclusivity)", async () => {
    ...
    const env = e2eLayer();
    const exit = await Effect.runPromiseExit(
      run(["node", "tsnuke", dir, "--deep", "--no-deep"]).pipe(Effect.provide(env.layer)),
    );
    expect(Exit.isFailure(exit)).toBe(true);
  });
...
// rejection to the DIE channel; `bin.ts` extracts its terse `tsnuke: <message>` via
...
describe("regression: engine error reaches the fail channel (real diagnoseNode)", () => {
  it("a directory WITHOUT tsconfig fails via the FAIL channel, not a defect", async () => {
    ...
    const env = e2eLayer();
    const exit = await Effect.runPromiseExit(
      run(["node", "tsnuke", dir]).pipe(Effect.provide(env.layer)),
    );
    expect(Exit.isFailure(exit)).toBe(true);
  ...
  it("the surfaced failure is an Error whose .message is bin.ts-printable (clean, no cause dump)", async () => {
    ...
    if (Exit.isFailure(exit)) {
      ...
      if (failure._tag === "Some") {
        ...
        // verbatim as `tsnuke: <message>`. Assert it's the clean human message, with
        ...
        expect(message).toContain("tsnuke analyzes TypeScript projects only");
