...
beforeEach(() => {
  dir = mkdtempSync(join(tmpdir(), "tsdoctor-install-"));
  out.length = 0;
});
afterEach(() => {
...
describe("planInstall (pure) — RULE-038 plan", () => {
  it("plans SKILL.md + pre-push always; agent-hook only with --agent-hooks", () => {
    ...
    expect(base.map((w) => w.path)).toEqual([
      "/x/.agent/skills/ts-doctor/SKILL.md",
    ...
    expect(withHooks.map((w) => w.path)).toContain("/x/.claude/hooks/ts-doctor.json");
  });
  ...
  it("buildSkillMarkdown is a real, deterministic SKILL.md", () => {
    ...
    expect(md).toContain("name: ts-doctor");
    expect(md).toContain("npx ts-doctor --format agent");
...
describe("runInstall over a real temp dir (Node FileSystem)", () => {
  it("writes the real SKILL.md + inert pre-push; returns 0", async () => {
    ...
    expect(code).toBe(0);
    const skill = join(dir, ".agent/skills/ts-doctor/SKILL.md");
    const hook = join(dir, ".git/hooks/pre-push");
    expect(existsSync(skill)).toBe(true);
    expect(readFileSync(skill, "utf8")).toContain("name: ts-doctor");
  ...
  it("--agent-hooks also writes the {} hook config", async () => {
    ...
    const cfg = join(dir, ".claude/hooks/ts-doctor.json");
  ...
  it("--dry-run writes NOTHING but describes the plan; returns 0", async () => {
    ...
    expect(code).toBe(0);
    expect(existsSync(join(dir, ".agent/skills/ts-doctor/SKILL.md"))).toBe(false);
