 ...
 *   - the legacy `import type` interfaces (`packages/tsnuke-rules/src/types.ts`), and
...
describe("Severity — canonical literal union accepts both engine values, rejects out-of-contract", () => {
  // RULE-031: tsnuke v1 has NO "info" level. All five vendored copies use the
...
describe("Diagnostic — canonical Schema is a SUPERSET of legacy + every vendored copy", () => {
  ...
  it("accepts the FULL shape with every optional present (url/fix/suppressionHint)", () => {
    expect(
      accepts(Diagnostic, {
        ...requiredFields,
        url: "https://tsnuke.dev/rules/no-ts-ignore",
        suppressionHint: "tsnuke-disable-next-line no-ts-ignore",
        fix: {
          kind: "auto-fix",
...
describe("Diagnostic — round-trip decode(encode(x)) === x", () => {
  it("round-trips a representative full Diagnostic", () => {
    const value: typeof Diagnostic.Type = {
      ...
      help: "Use unknown",
      url: "https://tsnuke.dev/rules/no-explicit-any",
      line: 42,
      column: 7,
      category: "types",
      tier: "TYP",
      ...
      suppressionHint: "tsnuke-disable-next-line no-explicit-any",
    };
