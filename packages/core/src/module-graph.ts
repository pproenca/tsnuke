 ...
 *   - `imports`     — resolved edges (for cycle/layering rules)
 ...
 *
 * Bare (package) imports are intentionally ignored — GRAPH rules reason about
 * the project's own module structure. Structural only: no `ts.Program` / checker.
 */

import { dirname, resolve } from "node:path";
import ts from "typescript";
import type { ModuleGraph } from "@ts-doctor/rules";
