...
Strangler-fig slice produced by `/code-modernization:modernize-transform tsnuke discovery effect`.
Source (READ-ONLY): `legacy/tsnuke/packages/core/src/discover-ts-project.ts:1-442`
...
(`packages/tsnuke-rules/src/types.ts:72`). Target:
`modernized/discovery/effect/` (package `@tsnuke/discovery-effect`).
...
- **RULE-022** (project discovery validity): `discoverTsProject` — **EFFECTFUL** over
  ...
  `@tsnuke/errors-effect` via a `file:` dep); a broken `package.json` stays a success
  with defaults.
...
`noUncheckedIndexedAccess` + `exactOptionalPropertyTypes`. The `@tsnuke/errors-effect`
...
| `Capability` alias | `tsnuke-rules/src/types.ts:72` (`type = string`) | `src/main/capabilities.ts` `Capability` (`= string`, parity) |
...
from `@tsnuke/errors-effect` (a `file:../../errors/effect` dep), so they carry the same
...
4. **`ProjectInfo` ownership (de-vendor target).** This slice OWNS `ProjectInfo` (it is the
   ...
   (config TRANSFORMATION_NOTES §7a) recommends a shared `@tsnuke/contracts-effect`
   ...
   of the canonical `Capability` from `@tsnuke/contracts-effect` (`Schema.String`,
...
- **`file:` dependency on the errors slice:** `"@tsnuke/errors-effect":
...
- **C1 — `Capability` de-vendor: DONE.** The local bare alias was deleted; this slice now
  imports the canonical `Capability` (type-only) from `@tsnuke/contracts-effect`. One
