 ...
 * (`legacy/tsnuke/packages/core/src/types.ts:22-51`). Modeling it as a Schema (not
 ...
 * `@tsnuke/contracts-effect` package may host shared domain types; if it does, this
 * `ProjectInfo` is the canonical source to de-vendor onto (see TRANSFORMATION_NOTES
 * Follow-up). The `Capability` token type (a `string` in legacy `@tsnuke/rules`)
