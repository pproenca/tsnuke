...
Strangler-fig slice produced by `/code-modernization:modernize-transform ts-doctor engine-plan effect`.
Source (READ-ONLY): `legacy/ts-doctor/packages/core/src/engine-plan.ts` (142 lines).
Target: `modernized/engine-plan/effect/`.

This is a **true strangler-fig**: the slice CONSUMES the already-completed
`capabilities` slice (`@ts-doctor/capabilities-effect`) for the activation predicate
...
dependency imports cleanly via the package name `@ts-doctor/capabilities-effect`
(`EnginePlan.ts`). Vitest transpiles the `.ts`-entry dependency at test time via
`vitest.config.ts → test.server.deps.inline: ["@ts-doctor/capabilities-effect"]`
(exactly as build-report inlines `@ts-doctor/score-effect`). No relative-import
fallback was needed.
...
| `resolveSeverity` (off→null, override, default) | `:59-65` (**private copy**) | **consumed** from `@ts-doctor/capabilities-effect` (see D1) |
...
| `RuleMeta`/`Severity`/`Capability`/`Tier` types | `:13-18` (`import type` from `@ts-doctor/rules`) | **consumed** from `@ts-doctor/capabilities-effect` (`Tier` = `RuleMeta["tier"]`) |
...
the one in `packages/ts-doctor-rules/src/capabilities.ts`. The two are
...
`@ts-doctor/capabilities-effect`. This removes the legacy duplication; it is
...
Legacy imports `Tier` from `@ts-doctor/rules`. The capabilities slice OWNS the `Tier`
...
- **`shouldActivate` / `resolveSeverity` were NOT re-exported** from this barrel —
  import them from `@ts-doctor/capabilities-effect` directly. This keeps the barrel's
...
5. **De-vendor the contract.** `RuleMeta`/`Severity`/`Capability` are re-exported from
   `@ts-doctor/capabilities-effect`, which itself vendors a subset pending the
   `@ts-doctor/rules` Effect slice (capabilities Follow-up #1). When `@ts-doctor/rules`
...
- **`file:` workspace dependency:** `package.json` declares
  `"@ts-doctor/capabilities-effect": "file:../../capabilities/effect"`. `pnpm install`
  links it; the package-name import (`from "@ts-doctor/capabilities-effect"`) resolves
  to the capabilities slice's `src/main/index.ts` (its `exports` entry).
- **Vitest `.ts`-dependency transpile:** `vitest.config.ts` sets
  `test.server.deps.inline: ["@ts-doctor/capabilities-effect"]` so esbuild compiles
