...
export default defineConfig({
  test: {
    ...
    // The `@ts-doctor/capabilities-effect` dependency is a `file:` link whose entry
    ...
    // Mirrors how build-report inlines `@ts-doctor/score-effect`.
    server: {
      deps: {
        // capabilities-effect now de-vendors its contract Schemas from
        // @ts-doctor/contracts-effect (also a `.ts`-entry `file:` dep), so inline both.
        inline: ["@ts-doctor/capabilities-effect", "@ts-doctor/contracts-effect"],
      },
    },
  },
});
