# `@tsnuke/engine-effect` — Transformation Notes

THE integrating keystone of the tsnuke modernization: the two-tier `runEngine`
...
- `legacy/tsnuke/packages/core/src/engine.ts:64-326` — `runEngine`, the two-tier shell.
- `legacy/tsnuke/packages/core/src/index.ts:154-249` — `diagnose()` + `readSourceFiles`
  + `overridesFromConfig`.
- `legacy/tsnuke/packages/core/src/types.ts` — `DiagnoseOptions`/`DiagnoseResult`/`ScoreResult`.
...
| `index.ts` `diagnose` (189-249) | `diagnose.ts` `diagnose` | `Effect<DiagnoseResult, TsNukeError, FileSystem \| Path \| Scope>`. Same wiring order. |
...
`contracts/effect` (`Diagnostic`, `Capability`, `TsNukeConfig`, `RuleMeta`),
