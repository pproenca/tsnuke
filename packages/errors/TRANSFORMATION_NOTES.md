...
Strangler-fig slice produced by `/code-modernization:modernize-transform ts-doctor errors effect`.
Source (READ-ONLY): `legacy/ts-doctor/packages/core/src/errors.ts` (61 lines).
Target: `modernized/errors/effect/` (package `@ts-doctor/errors-effect`).
...
- **Consequence:** a `ProjectNotFoundError` is **no longer `instanceof` a single
  ...
  actually performs.
- **How the guard preserves semantics:** `isTsDoctorError` is reimplemented as a
  **contract-based** check — `value instanceof Error && _tag ∈ TS_DOCTOR_ERROR_TAGS`
...
- **`AnyTsDoctorError` union type** replaces the base type for callers that want
...
2. **`build-report` slice (the CONSUMER) — KEEP `instanceof Error` + native `.cause`.**
   ...
   to a `_tag`-only check that would miss non-ts-doctor `Error`s in the chain. If
   it ever wants to special-case ts-doctor errors, use the exported
   `isTsDoctorError` / `TS_DOCTOR_ERROR_TAGS` rather than re-deriving the tag set.
3. **Replacing the `instanceof`-base habit:** any legacy call-site that did
   `if (e instanceof TsDoctorError)` must move to `if (isTsDoctorError(e))` (D2 —
   there is no shared base instance anymore). For exhaustive discrimination,
   `switch (e._tag)` over the `AnyTsDoctorError` union is the idiomatic form.
...
- **Guard ordering bug caught by the characterization tests:** the first guard
  ...
  `null`/`undefined`. The negative-input test (`isTsDoctorError(undefined)`) caught
...
**zero** `instanceof TsDoctorError` consumers, so dropping the shared base (D2) is not a
regression — `serializeError` only uses `instanceof Error` + native `.cause`.

**Applied:**
- **Drift guard added (MEDIUM).** `isTsDoctorError` discriminates via `_tag` membership of
  `TS_DOCTOR_ERROR_TAGS` (D2), a second source of truth that can silently diverge from the
  exported classes. `errors.test.ts` now asserts `TS_DOCTOR_ERROR_TAGS.size === <#classes>`
