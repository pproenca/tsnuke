...
Strangler-fig slice produced by `/code-modernization:modernize-transform ts-doctor exit-code effect`.
Source (READ-ONLY): `legacy/ts-doctor/packages/ts-doctor/src/exit-code.ts` (60 lines)
(+ the `FailOn` literal from `packages/ts-doctor/src/flags.ts:14` and `Severity` from
`packages/ts-doctor-rules/src/types.ts:13`). Target: `modernized/exit-code/effect/`.
...
- **`Diagnostic` was not vendored as a full Schema** (unlike the `score` slice). The
  ...
  it migrates to `@ts-doctor/rules` (its Follow-up #3).
...
2. **F2 — `config.failOn` is currently inert (consumer-wiring).** `config.failOn` is
   ...
   the CLI `--fail-on` flag, so a `failOn` set in `tsdoctor.config.json` never reaches
...
4. **F4 — de-duplicate `Severity` / `FailOn` ownership.** `Severity` is vendored both
   ...
   `@ts-doctor/rules` Effect slice lands (the `score` slice's Follow-up #3), both
