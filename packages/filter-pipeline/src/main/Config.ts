 ...
 * legacy `TsNukeConfig` plus `ConfigSeverity`/`IgnoreConfig`/`IgnoreOverride`. Those
 * are now consolidated in `@tsnuke/contracts-effect` (the canonical `effect/Schema`
 * home), whose `TsNukeConfig` is a proven structural SUPERSET of the old subset — so
