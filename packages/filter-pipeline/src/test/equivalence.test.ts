 ...
 *      legacy/tsnuke/packages/core/src/filter-pipeline.ts (218 lines), with the
...
import type { DiagnosticWithTags, TsNukeConfig } from "../main/index.js";

// ===========================================================================
// ORACLE — frozen verbatim copy of
// legacy/tsnuke/packages/core/src/filter-pipeline.ts (READ-ONLY source).
// Attribution: tsnuke @tsnuke/core, src/filter-pipeline.ts. The legacy
...
function legacyMakeSeverityStage(config: LegacyTsNukeConfig): LegacyStage {
...
function legacyMakeIgnoreStage(config: LegacyTsNukeConfig): LegacyStage {
...
const LEGACY_DISABLE_NEXT_LINE_RE = /\/\/\s*tsnuke-disable-next-line\s*(.*)$/;
...
const SRC_DISABLE_RULE = new Map([
  [
    "/x/a.ts",
    ["const a = 1;", "// tsnuke-disable-next-line no-magic", "const b: any = 2;"].join("\n"),
  ],
]);
...
const SRC_DISABLE_NS = new Map([
  ["/x/a.ts", ["// tsnuke-disable-next-line tsnuke/no-magic", "const b = 2;"].join("\n")],
]);
const SRC_DISABLE_LIST = new Map([
  ["/x/a.ts", ["// tsnuke-disable-next-line  a, b ,c", "const b = 2;"].join("\n")],
]);
const SRC_CRLF = new Map([
  ["/x/a.ts", "// tsnuke-disable-next-line r\r\nconst b = 2;\rconst c = 3;"],
]);
...
const fixtures: Fixture[] = [
  ...
  { name: "rules off ns", diagnostics: [diag({ rule: "off-me" })], config: { rules: { "tsnuke/off-me": "off" } } },
  ...
  { name: "rules collision: bare warn beats ns off", diagnostics: [diag({ plugin: "tsnuke", rule: "r", severity: "error" })], config: { rules: { r: "warn", "tsnuke/r": "off" } } },
  { name: "rules collision: bare off beats ns warn", diagnostics: [diag({ plugin: "tsnuke", rule: "r", severity: "error" })], config: { rules: { r: "off", "tsnuke/r": "warn" } } },
  ...
  { name: "ignore rule ns", diagnostics: [diag({ rule: "ig" })], config: { ignore: { rules: ["tsnuke/ig"] } } },
  { name: "ignore rules collision: both forms present still drops", diagnostics: [diag({ plugin: "tsnuke", rule: "ig" })], config: { ignore: { rules: ["ig", "tsnuke/ig"] } } },
  ...
  { name: "override with rules ns", diagnostics: [diag({ rule: "scoped", filePath: "/x/c.ts" })], config: { ignore: { overrides: [{ files: ["c.ts"], rules: ["tsnuke/scoped"] }] } } },
  { name: "override rules collision: both forms present still drops", diagnostics: [diag({ plugin: "tsnuke", rule: "scoped", filePath: "/x/c.ts" })], config: { ignore: { overrides: [{ files: ["c.ts"], rules: ["scoped", "tsnuke/scoped"] }] } } },
  ...
  { name: "inline disable ns", diagnostics: [diag({ plugin: "tsnuke", rule: "no-magic", filePath: "/x/a.ts", line: 2 })], config: {}, options: { sources: SRC_DISABLE_NS } },
