...
Strangler-fig slice produced by `/code-modernization:modernize-transform tsnuke fix-applier effect`.
Source (READ-ONLY): `legacy/tsnuke/packages/tsnuke/src/fix-applier.ts:36-244` — the
...
`modernized/fix-applier/effect/` (package `@tsnuke/fix-applier-effect`).
...
`Diagnostic`/`Fix`/`TextEdit` are imported from `@tsnuke/contracts-effect` (`file:`
...
3. **Atomic write** — write to an **unpredictable** same-dir temp
   (`.<base>.<seq>.tsnuke-fix.tmp`) opened **O_EXCL (`flag: "wx"`, SEC-002 hardening)** —
...
- **SEC-002 (MEDIUM) — the cure's own predictable temp file was a new symlink-write sink.**
  The temp was a fixed `.<base>.tsnuke-fix.tmp` written with the default `"w"` flag (which
  follows symlinks), so a pre-planted symlink there could be clobbered. **Fixed:** the temp
  is now unpredictable (`.<base>.<seq>.tsnuke-fix.tmp`) and opened **O_EXCL (`flag: "wx"`)**,
