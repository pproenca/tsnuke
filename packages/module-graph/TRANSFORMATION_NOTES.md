...
`/code-modernization:modernize-transform tsnuke module-graph effect`.

Source (READ-ONLY): `legacy/tsnuke/packages/core/src/module-graph.ts` (215 lines).
Target: `modernized/module-graph/effect/` (package `@tsnuke/module-graph-effect`).
...
`exactOptionalPropertyTypes` + `verbatimModuleSyntax` · the `@tsnuke/rules-core-effect`
...
| `import type { ModuleGraph } from "@tsnuke/rules"` | `import type { ModuleGraph } from "@tsnuke/rules-core-effect"` (now OWNED by rules-core) |
...
The legacy file imported `ModuleGraph` from `@tsnuke/rules`; the modernized type is OWNED
by `@tsnuke/rules-core-effect` (single-site GRAPH-tier input, deliberately NOT in the
shared `@tsnuke/contracts-effect`). We `import type { ModuleGraph }` from rules-core —
...
owner, `@tsnuke/rules-core-effect` (mirrors the rules-core / type-performance barrel
...
- **`ModuleGraph` not copied** — consumed read-only as a type from `@tsnuke/rules-core-effect`.
...
- **`vitest.config.ts`** inlines `@tsnuke/rules-core-effect` + `@tsnuke/contracts-effect`
