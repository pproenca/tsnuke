...
Strangler-fig slice produced for the tsnuke modernization. Builds the **rule
...
Source (READ-ONLY): `legacy/tsnuke/packages/tsnuke-rules/src/`
...
Target: `modernized/rules-core/effect/` (package `@tsnuke/rules-core-effect`).

This slice is the **FIRST NEW consumer of `@tsnuke/contracts-effect`**: it
...
| Behavior | Legacy `tsnuke-rules/src/…` | Target |
|----------|--------------------------------|--------|
| `PLUGIN_NAME = "tsnuke"` (BC-18) | `define-rule.ts:5` | `src/main/defineRule.ts` |
...
| `Diagnostic` / `RuleMeta` / `Severity` / `Tier` / `FixKind` | `types.ts` (owned by rules) | **imported** from `@tsnuke/contracts-effect` |
...
Legacy `tsnuke-rules` OWNED these types in `types.ts`. The modernization
consolidated the cross-cutting domain contracts into `@tsnuke/contracts-effect`
...
- **`file:` workspace dependency:** `package.json` declares
  `"@tsnuke/contracts-effect": "file:../../contracts/effect"`. `pnpm install` links
  it; the package-name import (`from "@tsnuke/contracts-effect"`) resolves to the
  contracts slice's `src/main/index.ts` (its `exports` entry). This is the same
  `file:`-dep pattern build-report uses for `@tsnuke/score-effect`.
...
- **Vitest `.ts`-dependency transpile:** `vitest.config.ts` sets
  `test.server.deps.inline: ["@tsnuke/contracts-effect"]` so esbuild compiles the
...
say "vendored contract" where it is now imported from `@tsnuke/contracts-effect`
