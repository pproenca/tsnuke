...
/**
 * CFG (inverted gating, RULE-020) — recommend enabling tsconfig `strict`.
 *
 * This rule does NOT inspect the file AST. It is evaluated at the project level
 * by the activation predicate (the capabilities slice's `shouldActivate`):
 * `requires:["tsconfig"]` (a tsconfig exists) and `disabledBy:["strict"]` (the
 * `strict` capability token is present when the flag is already ON). So it fires
 * *only when `strict` is OFF* — and self-disables the moment the goal is met.
 * `create` is intentionally a no-op; core surfaces the project-level finding from
