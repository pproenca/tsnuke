...
This is the first slice to **CONSUME `@tsnuke/contracts-effect`** rather than
...
are the oracle (`legacy/tsnuke/packages/tsnuke-rules/src/{define-rule,identity}.ts`
...
| BC-18 | `plugin` forced to `"tsnuke"` on every emitted diagnostic | `createRuleContext.test.ts`, `equivalence.test.ts` |
...
- `plugin` is **forced** to `"tsnuke"` (cannot be overridden — not in `ReportInput`).
...
Vitest must transpile the `.ts`-entry `@tsnuke/contracts-effect` dependency at
test time; `vitest.config.ts` sets `test.server.deps.inline:
["@tsnuke/contracts-effect"]` (the `file:` link's `exports` is
`./src/main/index.ts`).
...
// import them from @tsnuke/contracts-effect (this slice consumes, not vendors).
```
