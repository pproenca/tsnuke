...
Strangler-fig slice produced by `/code-modernization:modernize-transform ts-doctor declaration-api effect`.

Source (READ-ONLY): `legacy/ts-doctor/packages/ts-doctor-rules/src/rules/declaration-api/`
(4 SYN rules + their colocated `*.test.ts`).
Target: `modernized/rules-declaration-api/effect/` — package `@ts-doctor/rules-declaration-api-effect`.
...
(`@ts-doctor/rules-core-effect` for `defineRule` / `runRule` / `Rule` / `RuleContext`) and the
canonical data contracts (`@ts-doctor/contracts-effect` for `Diagnostic` / `RuleMeta`, reached
...
(`@ts-doctor/rules-core-effect`) and `file:../../contracts/effect`
(`@ts-doctor/contracts-effect`) import cleanly via package name. `typescript` is a real runtime
...
time via `vitest.config.ts → test.server.deps.inline: ["@ts-doctor/rules-core-effect",
...
- **No vendored substrate / contracts.** This slice CONSUMES `@ts-doctor/rules-core-effect`
  (`defineRule`, `runRule`, `Rule`, `RuleContext`) and `@ts-doctor/contracts-effect`
