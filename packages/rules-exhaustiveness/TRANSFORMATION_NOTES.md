...
Strangler-fig slice produced by `/code-modernization:modernize-transform ts-doctor exhaustiveness effect`.

Source (READ-ONLY): `legacy/ts-doctor/packages/ts-doctor-rules/src/rules/exhaustiveness/`
(8 rules — 3 SYN + 5 TYP — plus their colocated `*.test.ts`).
Target: `modernized/rules-exhaustiveness/effect/` — package `@ts-doctor/rules-exhaustiveness-effect`.
...
already-completed substrate (`@ts-doctor/rules-core-effect` for `defineRule` / `runRule` /
`runTypeAwareRule` / `Rule` / `RuleContext`) and the canonical data contracts
(`@ts-doctor/contracts-effect` for `Diagnostic` / `RuleMeta`, reached transitively through
...
`vitest.config.ts → test.server.deps.inline: ["@ts-doctor/rules-core-effect",
...
`@ts-doctor/rules-core-effect` instead of the legacy relative `../../define-rule.js`.
...
imported from `@ts-doctor/rules-core-effect`. **Every legacy `*.test.ts` case was ported**
...
- **Consumes the shared substrate + contracts.** The rules import `defineRule` / `RuleContext`
  from `@ts-doctor/rules-core-effect`, and the tests import `runRule` / `runTypeAwareRule` from
  the same package — instead of the legacy relative `../../define-rule.js` / `../../test-utils.js`.
  The `Diagnostic` / `RuleMeta` contracts come transitively from `@ts-doctor/contracts-effect`.
