...
`/code-modernization:modernize-transform tsnuke rules-graph effect`.

Source (READ-ONLY): `legacy/tsnuke/packages/tsnuke-rules/src/rules/` (the two GRAPH
...
Target: `modernized/rules-graph/effect/` (package `@tsnuke/rules-graph-effect`).

Implements tsnuke's **2 GRAPH-tier rules**: **RULE-015** (`no-import-cycles` — 3-color
...
| `runGraphRule` test driver | the `run(graph)` helper in legacy `*.test.ts` | imported from `@tsnuke/rules-core-effect` (not vendored) | `src/test/*.test.ts` |
...
- `defineGraphRule` is imported from `@tsnuke/rules-core-effect` instead of the legacy
...
- `runGraphRule` (the GRAPH driver — the legacy tests' inline `run(graph)` helper that builds
  ...
  `@tsnuke/rules-core-effect` in the tests, so the tests exercise the REAL production
  driver — the same `createGraphRuleContext` + `analyze` pass the engine uses on the GRAPH
  path — not a test-only copy.
- `ModuleGraph` is imported as `import type` from `@tsnuke/rules-core-effect` (it OWNS the
...
- `Diagnostic` / `RuleMeta` come from `@tsnuke/contracts-effect` transitively (the rules
...
`package.json` adds `@tsnuke/rules-core-effect` **and** `@tsnuke/contracts-effect` as
...
& `{ analyze }`) flows from rules-core. The direct `@tsnuke/contracts-effect` dep is therefore
