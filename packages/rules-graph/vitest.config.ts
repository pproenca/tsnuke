...
export default defineConfig({
  test: {
    ...
    // `defineGraphRule` / `runGraphRule` / `createGraphRuleContext` are runtime VALUES from
    // rules-core, and rules-core itself imports contracts-effect, so contracts must be
    // inlined too. Same pattern the module-boundaries / type-performance slices use.
    server: {
      deps: {
