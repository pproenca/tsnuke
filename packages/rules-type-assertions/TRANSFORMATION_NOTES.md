...
Strangler-fig slice produced by `/code-modernization:modernize-transform ts-doctor type-assertions effect`.
Source (READ-ONLY): `legacy/ts-doctor/packages/ts-doctor-rules/src/rules/type-assertions/**`
(12 SYN rules + 1 TYP rule + their colocated `*.test.ts` behavioral specs). Target:
`modernized/rules-type-assertions/effect/` (package `@ts-doctor/rules-type-assertions-effect`).
