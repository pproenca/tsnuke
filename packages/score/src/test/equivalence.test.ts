...
// ---------------------------------------------------------------------------
// ORACLE: Frozen copy of legacy/ts-doctor/packages/core/src/score.ts:49-69
...
function buildDiagnostics(e: number, w: number): Diagnostic[] {
  ...
  for (let i = 0; i < e; i++) {
    out.push({ plugin: "ts-doctor", rule: `err-${i}`, severity: "error" } as Diagnostic);
  }
  for (let i = 0; i < w; i++) {
    out.push({ plugin: "ts-doctor", rule: `warn-${i}`, severity: "warning" } as Diagnostic);
  }
  return out;
}
